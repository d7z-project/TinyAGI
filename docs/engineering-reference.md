...
Secret 的引用、用户归属、创建／提交者、用途、授权、提交请求及状态元数据可放业务 SQLite，沿统一资源类型提供受限投影；原值进入独立受限的本地机密存储，不作为可通用读取的内容，不纳入全文／向量索引或副本导出。存储采用受保护的密文或操作系统凭据设施，解锁材料与普通数据／导出分开；具体库与平台适配尚未锁定，不把同目录保存密文及明文主密钥称为完成保护。无需为此部署独立云端密钥服务。
...
逻辑上的提交、接续和外部不确定性见[运行契约](runtime-protocol.md#commit)。以下说明 SQLite、资源类型接入、文件与恢复机制。

<a id="resource-integration"></a>

统一资源的[公共属性和类型规则](projection-input-and-compute.md#resource-contract)在 Go 宿主中由对应状态所有者与类型处理适配实现。SQLite 保存业务描述及关联，大段内容继续使用本地文件；外部材料保存受管理引用，向量与 KV 按原提供者契约处理。具体字段、接口签名及初始类型清单在接入时确定，不增加第二套资源数据库或独立策略服务。

目录／索引可汇总描述，投影由可信请求上下文和当前状态生成；通用描述不能直接序列化为模型输入。来源、内容修订和处理范围关联缓存及派生产物，正文与机密原值仍沿相应内容通路处理。Secret 的保管和注入见[机密工程映射](#maintenance-secrets)，不因统一引用而允许通用文件读取原值。

资源类型、表示范围及操作用途映射为有类型的接口参数，复用既有 MRPC 生成和实现绑定；Go／Rust／Node.js 能力端遵守宿主准入及返回契约。普通资源引用不包含认证访问凭证，受信执行职责按目标操作解析使用；类型、处理实现和动态页面描述一致采用。此为宿主接入设计，不把通用资源权限或完整信息隔离声明为 go-mini 已有能力。
...
| 数据 | 放置 |
| --- | --- |
| 资源描述、类型及来源关联 | SQLite 中由各原所有者维护；共同目录为查询视图或派生索引，受限描述按身份投影 |
