...
| 统一资源 | 资源专题维护公共属性、类型规则、情境投影、表示、派生和使用记录；运行协议维护操作接纳 | 管理专题提供同源详情及类型操作，沙箱维护范围装配，工程参考维护类型接入 |
| 机密 | 资源专题维护 Secret 类型规则，管理专题维护用户归属、可信提交及处理交互，运行协议维护专用调用 | 复用共同资源定义，工程参考说明原值存储与适配，不另建一套投影语义 |
...
统一用“沙箱”，Secret 中文统称“机密”。资源、引用、描述、内容表示、情境投影及 Secret 类型规则沿资源专题定义；机密归属与可信提交沿管理专题，实际调用沿运行协议。投影不是全部资源描述或已读证明，读取、执行使用和交付分别表达；“程序／配置修订”不等于“产品版本”，逻辑模块不是部署进程，KV 属于推理计算组件。
