...
| 阅读主题 | 章节入口 |
| --- | --- |
| 职责与能力 | [逻辑模块](#software-modules) · [检索责任](#vector-service) · [公共能力 API](#capability-api) · [资源操作](#resource-operations) · [机密使用](#secret-contract) · [模型端口](#model-port) |
| 程序与采用 | [受管理函数](#managed-functions) · [配置生效](#configuration-resolution) · [能力构建与运行](#capability-lifecycle) · [实现信任](#implementation-trust) · [隔离执行](#isolated-execution) |
...
| 世界与记忆 | 记忆材料、来源、断言和派生检索关系 | 有范围的发现／读取、索引任务、候选核对及带依据的修订 |
| 共同资源机制 | 各原所有者提供的资源描述、类型规则和情境视图 | 统一发现与投影，操作路由至原所有者；目录不接管内容或业务状态 |
...
[资源专题](projection-input-and-compute.md#resource-contract)定义公共属性、类型规则、表示及情境投影。本节只维护请求、实际操作和结果接续：发现／描述、读取、专用使用、转换、交付和维护具有明确语义，不建设一个可任意读取原值或修改业务状态的通用入口。

| 阶段 | 运行责任 |
| --- | --- |
| 提出请求 | 明确资源引用、必要修订／表示／范围、具体操作、用途及结果去向；沿原活动、任务和预算关联 |
| 核对与接纳 | 依据可信身份及执行域，核对原所有者的当前描述、类型限制、策略、授权、处理组件及控制；调用者自报用途不产生权限 |
| 执行与使用 | 实际实现须具备该操作及相应信任资格，按已接纳范围读取、加载或专用使用；敏感内容只经符合类型约束的通路传递 |
| 返回与派生 | 区分内容来源和运行使用关系；按契约分类、转换和过滤，保留真实来源、范围、状态及已知用量 |
| 接续与交付 | 结果先归原操作，再按当前范围形成可见内容、产物引用或状态事件；实际到达接收点与生成产物分开记录 |

类型声明、支持操作与投影中的可用提示不授予执行权限。内容修订、描述修订和当前授权分别处理；代码与普通行为配置仍随执行固定，许可、Secret 资格、额度及控制在相应准入／使用／交付入口核对。元数据编辑不能取消强制类型限制，例外需由有权职责调整对应约束。

资源类型可声明结构、操作说明和返回类别，处理实现、类型规则及调用描述一致采用；生成表单与蓝图复用该来源。资源参数保留类型、预期用途和范围，外部实现的权限绑定实际产物及执行条件，不从同名接口或测试通过推导原值访问权。跨语言映射见[工程参考](engineering-reference.md#resource-integration)，不把宿主设计声明为执行库已有机制。

步骤内读取可独立接纳并返回内容，本步骤记录实际依据；长读取、索引或转换保存操作引用并释放认知入口。最终提案只引用已接纳操作，不重复派发。外部任务控制覆盖原范围内的在途工作与迟到结果；单次认知中断仍不自动取消独立操作。[接纳时序](#step-operation-admission)、[任务控制](#task-control)

发现不到、没有读取资格、表示不支持、内容尚未取得和处理失败分别处理；对外说明本身也受可见性限制。外部资源引用可以已登记而正文尚未取得，不能作为已读依据或本地产物发布完成的证明。有效已读材料可以保留或按需复用，具体记录见[读取与使用](projection-input-and-compute.md#resource-reading)。
...
Secret 沿[统一资源操作](#resource-operations)核对，类型及引用定义见[资源专题](projection-input-and-compute.md#secret-resource)。机密职责保管原值，维护用户归属与独立使用绑定；原值及入口凭证在通用聊天、事件载荷、日志和模型预处理前分流。统一事件仅表达获准引用、请求标识及状态；不提供向认知返回 SecretValue 的工具或 FFI。动态代码与沙箱不能通过通用文件、环境或请求调试绕过类型限制，专用指令与用户交互由[管理专题](management-workspace.md#secret-command)维护。
...
SecretRef 只作标识，宿主依据当前归属、调用者、实际能力实现、执行域、目标提供者及允许操作核对绑定；受信适配器在协议认证位置使用原值，不经提示模板或蓝图参数文本替换。绑定不允许任意端点、重定向或回显用途。认证属于运行使用关系，业务结果按目标契约分类；认证材料、错误回显及敏感派生内容在进入认知、普通事件和日志前隔离，无法安全分离的载荷留在受限通路。别名及归属变更沿[机密归属规则](management-workspace.md#secret-ownership)处理。
...
构建和运行前核对执行环境是否具备所要求的文件、网络、状态及资源约束。缺少必要约束时不启动该范围的候选，返回环境能力不足；可以改用已有合格能力或缩小任务，但不能自动回落普通执行环境。预制[隔离执行能力](#isolated-execution)统一承接所需约束及环境选择，具体承载见[工程装配](engineering-reference.md#container-execution)；环境独立本身不证明全部约束已经满足。
...
宿主依据具体实现、来源、用途、输入材料、实际作用及当前策略确定必要约束。第三方或动态生成的可执行候选、开放命令、依赖准备脚本，以及处理不可信复杂输入或承担高风险远程操作的实现，须在合格隔离环境中执行。要求覆盖准备、构建、测试和正式运行；本地调用、远程调用、包装函数和反向调用均不能成为豁免依据。受信预制实现可使用符合其用途的普通路径；候选通过测试不自动取得该资格。

| 契约内容 | 权威来源及处理 |
| --- | --- |
| 归属与范围 | 宿主绑定调用者、Episode／Task／Operation、实际实现及真实或试验执行域，候选不能自报更高权限 |
...
环境可见属性与运行要求沿[资源契约](projection-input-and-compute.md#resource-actions)表达，属性编辑不产生授权。Self 可申请更多资源或提出替代方案，不能通过更换名称、包装、实现或执行入口规避仍有效的要求。具体隔离机制、运行时及系统权限由[工程参考](engineering-reference.md#container-execution)维护。
...
一次构建、任务执行或受管理能力会话可以包含多次调用，无需为每个请求重建环境。复用须保持身份、权限、数据范围和实现条件；不同用户、试验分支或信任范围默认不共享可变环境。已有临时资源关联创建它的实现与运行对象，不因重新绑定或重建环境被视为已迁移。下属工作共同计入父活动和全局预算，环境数量不增加额度。

环境不持有主体状态的直接维护权，只获得授权材料和独立产物范围。日志、文件和构建结果沿资源体系返回状态与投影；显式读取才进入认知。结果仍作为有来源的外部输入核对，取出产物不自动授权执行，隔离成功也不证明内容可信。

Secret 默认只提供引用；受信适配器优先在专用认证位置使用原值，目标及用途绑定不得退化为任意带凭据请求。确需执行实现处理原值时，同时核对该实现资格和当前用户授权，不因隔离而额外授予权限。环境不继承完整主体材料、宿主凭据或管理入口；认证回显与敏感派生内容仍先经过[机密管线](management-workspace.md#secret-pipeline)。

取消沿[任务控制](#task-control)覆盖所属执行和子工作，分别记录请求发出、正式接纳、实际停止和资源收尾；终止执行不证明外部服务已取消或现实效果已撤销。独立远端服务的内部执行按提供者契约说明，本地环境的限制不能作为远端隔离证明。可用性和不足原因由[工作台](management-workspace.md#execution-environments)展示，真实任务与沙箱共用此契约。
...
图中分发是逻辑责任，不要求单条 FIFO 或全量广播。确定回执、普通进度及控制由机制处理，只有影响判断、计划或交付的事项才触发相应认知；相关事件可汇集处理。停止等实时控制走宿主优先通路，不等待慢模型；经授权可先发出止动请求，正式接纳和实际停止沿[任务控制契约](#task-control)分别确认。模型与工具结果先归入操作事实及事件，再接续活动，结果先确认归属，再由活动决定接续。
...
StepContext 组织本次可见输入：活动与执行域、代码身份、令牌／期限、输入事件和消费位置、连续状态、情境投影／曾读材料、本次实际选入内容、开放问题、在途操作、能力与预算。工作区是业务状态，模型上下文是当前选择的临时视图。执行使用和实际交付保留在对应操作记录，不因组件用过某资源就记作模型已读。
...
未确认正式接纳时，如实报告接纳待确认或失败；即时止动的成功不能代替任务状态，也不承诺重启后仍保留该阻断或中止。正式接纳的保存与回执顺序见[工程映射](engineering-reference.md#commit)。
...
| 受管理资源的描述及内容 | 各原所有者维护公共属性、类型规则、内容表示和派生关系；共同目录只汇总，投影按当前情境生成 |
| Secret 原值、归属、提交与绑定 | 机密职责保存原值和用户归属，受信输入职责处理提交；共用资源引用与投影，通用读取、事件、记忆和模型端口均不携带原值或入口访问凭证 |
...
本地产物只有内容已发布后才能登记为已可用引用；外部资源可登记受管理引用，并明确内容未取得或当前可用性未知，不冒充本地发布成功。保留、删除和派生副本清理各有责任，覆盖不足或来源缺失须可见。记录格式、数据库与目录布局见[工程参考](engineering-reference.md#storage)。
