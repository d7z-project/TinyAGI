...
| 专项检查 | [初始化与迁移](#initialization-evaluation) · [预制能力](#prebuilt-evaluation) · [任务解耦与控制](#task-control-evaluation) · [跨契约一致性](#contract-consistency-evaluation) · [隔离执行](#isolated-execution-evaluation) |
...
沙箱复用[统一资源契约](projection-input-and-compute.md#resource-contract)：分别装配可见描述、允许读取的内容、可写状态及测试使用资格。原所有者的当前约束继续有效；共享只读材料不复制维护权，资源类型或运行使用也不授予正文回读。具体副本重建范围见下节建议。

认知试验与[受控隔离执行](runtime-protocol.md#isolated-execution)分别装配：前者决定人物状态、替代能力、输入与评价，后者约束程序能接触的系统资源及实际投入。真实任务也可使用隔离执行；创建试验分支不自动满足系统隔离要求，启动独立环境也不代表完整 AGI 副本已创建。工程承载见[容器执行](engineering-reference.md#container-execution)。

| 范围 | 当前默认规则 |
| --- | --- |
| 状态写入 | 工作区、记忆、世界断言、人格情感、关系评价、计划和待办写入本沙箱；不能修改原活动或其他试验 |
| 资源发现与读取 | 初始按试验情境形成所选资源的投影；读取表示和范围须获准。类型规则和当前外部处理许可继续有效，已删除或无权访问的材料保持缺失 |
...
| 场景 | 必须保持的契约 |
| --- | --- |
| 同一资源用于管理、认知和试验，期间可见范围改变 | 按当前接收方形成投影；旧引用或缓存不延长权限，管理读取不自动注入认知 |
| 已获准操作使用凭据并返回业务内容 | 凭据沿专用绑定使用；结果按内容来源及目标契约处理，不将全部业务结果误标为 Secret，也不把认证回显交给模型 |
...
<a id="isolated-execution-evaluation"></a>

### 8.8 可选隔离执行的评价规格

以下规格用于核对隔离能力与既有主体、资源及控制契约的衔接，不是已执行测试或新增实验任务。具体运行时的适用条件由[工程资料](engineering-reference.md#container-sources)说明，无法据此认定本项目的隔离效果或性能已测。
...
| 执行申请 Secret 或返回认证回显 | 引用不授予原值访问，认证按当前用途及实现资格处理；日志与产物先过滤，模型只见获准投影或选读内容 |
| 控制台中止或环境失联，远端结果随后到达 | 请求、接纳与实际停止分别显示；结果归原任务，不自动重跑、不据本地停止声称远端效果已撤销 |
| 请求增强隔离但只有较弱环境 | 返回缺失能力，不降低要求；认知沙箱的模拟成功不替代所需执行限制 |

资料、实际环境检查、隔离测试和认知质量评价分别记录。完整副本重建仍沿其独立建议状态，不因新增执行能力而推定已完成。
...
| 试验状态与隔离执行分别装配 | 复用预制执行能力约束候选，保持试验输入、业务状态及评价归属；不将环境启动当作副本完整或测试通过 |
