...
[文档索引](README.md) · [总设计](../DESIGN.md#sandbox) · [观测与学习](../DESIGN.md#learning) · [设计收尾与边界](research-plan.md#remaining-questions)

目录：[目标](#purpose) · [职责与架构](#architecture) · [使用方式](#modes) · [隔离与能力](#isolation) · [AGI 副本](#replica) · [函数测试入口](#function-evaluation) · [试验分层与构建环境](#test-scopes) · [完整流程](#lifecycle) · [复盘与采用](#review) · [能力评估设计](#evaluation-design) · [覆盖矩阵](#capability-matrix) · [用例与测试集](#test-cases) · [基线对照](#baselines) · [评价器](#graders) · [退化与采用](#regression-gates) · [报告与触发](#evaluation-operations) · [主动学习评估](#learning-evaluation) · [初始化与迁移评估](#initialization-evaluation) · [预制能力评价](#prebuilt-evaluation) · [任务解耦与控制评价](#task-control-evaluation) · [跨契约检查](#contract-consistency-evaluation) · [选择与依据](#sources) · [收尾与适用边界](#validation)
...
可运行副本是用户明确要求；以下为具体机制建议，状态见[设计台账](research-plan.md#management-extension)。副本的状态继承、能力隔离和采用由本专题维护，页面编辑由[工作台](management-workspace.md#revision-review)维护。
...
中间状态也按当时责任核对。报告 016 的评分脚本仅检查通知最终状态，接受了创建成功就提前done、之后才提醒的轨迹；单独审阅将其保留为评分盲区，原分数不覆盖。报告 017 已以新内容身份加入这一已知反例并重新校准；原评分器与旧分数保留，不将新检查追写成旧能力。
...
学习者可生成开发练习；独立保留任务的答案和评价控制仍由评估职责管理。同源自评或更换评分模型不自动构成独立依据，练习材料泄入保留评价时标记比较受影响。候选可提出新的评价方法，但不能修改自己本轮的评价条件和结果；评价方法另作独立候选检查。
...
未来明确要求验证时可先使用真实 mini-go 与确定性受控提供者覆盖上述主流程，无需用真实模型证明取消和入口行为。库用例阅读、原报告 014 的顺序接续与未来完整场景分别标注；本轮未运行测试、未给出停止延迟或响应性成绩，不自动新增实验队列。来源见[库事实](go-mini-integration.md#task-control-facts)及[工程映射](engineering-reference.md#task-execution)。
...
这些规则的采用理由、替代项与确认范围见[一致性决策](research-plan.md#architecture-consistency)，不新增底层故障证明要求。
...
机械隔离检查和认知效果分别报告；配置违规的运行保留轨迹，但不作为有效公平样本。初始状态、能力装配、环境覆盖、评阅采用与迁移分别归入 Q1、Q3、Q5、Q6。执行不可信原生候选必须满足已定义的环境准入与实现信任约束；本设计规定拒绝不满足约束的执行路径，不据此声称任何具体隔离实现已经验证。
