...
本篇统一维护设计状态、原 Q1–Q7 的决定、管理工作台建议、未采用方案及用途配置。Q 编号和文件路径保留用于历史引用，不是产品版本、实施里程碑或待执行实验清单。

目录：[当前范围](#research-status) · [收尾状态](#closure-status) · [完整任务](#whole-system) · [决策台账](#remaining-questions) · [用途配置](#deferred) · [管理设计状态](#management-extension) · [能力扩展设计](#capability-extension)／[Node.js 补充](#node-rpc-extension) · [主动学习设计](#active-learning-extension) · [初始化与迁移](#initialization-extension) · [预制能力库](#prebuilt-extension) · [核心与任务控制](#task-control-extension) · [架构一致性](#architecture-consistency) · [后续维护](#next-steps)
...
| 状态 | 适用对象 | 含义 |
| --- | --- | --- |
| 已确认要求 | 统一主体、强自主、统一事件、投影、完整管理、可信机密输入等用户要求 | 必须在架构与专题保持一致；不表示已实现 |
| 已定设计 | 原 Q1–Q7，以及已认可的编写契约、调用边界热加载、能力构建与多语言接入、主动学习扩展及强度控制、提示词初始化与个体自迁移、客户端预制能力库、核心／任务解耦及外部控制、本次整体审计认可的跨契约澄清 | 已有资料或局部证据支持，可作为继续设计及未来实现的依据 |
| 设计建议 | 管理工作台中尚未单独确认的副本重建细则 | 方案已写明，保留审阅状态；整理不将其改为用户已认可 |
| 用途配置 | 模型、设备、预算、联系窗口和评价门槛等 | 随具体用途确定，不是未关闭架构问题 |
| 历史证据 | 报告、脚本、输入、失败与原始数据 | 仅支持其固定条件，不是当前任务队列 |

原 Q1–Q7 队列没有未关闭项。实际能力画像、性能和真人体验尚无完整测量，这是证据边界；管理设计建议单独列示，不能以原队列关闭推断所有新增细节均获确认。
...
| --- | --- | --- |
| 模型、SDK、服务与设备版本 | 用户明确进入对应接入时，由工程配置固定 | 声明能力、处理范围、费用口径与终止语义；不以同名／同结构推定等价 |
...
| 能力门槛、样本和基线 | 用户以后要求能力测量时，在查看候选成绩前确定 | 质量／成本及关键失败分维度，保留未知和失败，不复用公开调参样本冒充保留任务 |
...
| 范围 | 状态与唯一正文 |
| --- | --- |
| 完整管理、模型／服务维护 | 用户已认可／明确要求；[管理职责与范围](management-workspace.md#coverage) |
| 普通用户的机密归属、专用指令、可信提交及过滤转换 | 用户明确要求；[机密机制](management-workspace.md#secret-projection)，具体命令名及字段为示意 |
| 可运行 AGI 副本 | 用户明确要求；基准重建、创建后暂停、分支操作及有限采用为[设计建议](sandbox-evaluation.md#replica) |
| 动态表单、蓝图及代码编辑 | 类型推导、结构化注释和自动生成适配已获认可；[编辑映射](management-workspace.md#dynamic-editing)，具体语法与组件未冻结 |
| 提交立即生效 | 按编写契约准备候选、调用边界切换、等待或撤销计算后接续、失败保留旧内容已获认可；[生效契约](management-workspace.md#apply) |
| 受管理函数与热加载 | 已认可：宿主管长期状态、稳定函数标识、有界调用、类型与注释描述、生成入口及沙箱测试；[编写与调用契约](runtime-protocol.md#managed-functions)、[工程映射](engineering-reference.md#managed-function-tooling) |

资料与未采用方案见[管理依据](management-workspace.md#sources)。已认可的热加载方案降低手工适配及任意栈恢复的设计负担；收益是工程判断，未量化性能或开发成本。副本细则仍独立保留建议状态。这里的认可不重开原七项问题，也不授权运行实验或实现产品。

<a id="capability-extension"></a>

## 能力构建与接入的确认范围

基础方案于 2026-09-20 获用户认可，Node.js（npm）范围于 2026-09-21 补充；以下为当前已定设计，不授权实现、模型调用或新增实验：

| 范围 | 采用方案与唯一正文 | 未采用方向与理由 |
| --- | --- | --- |
| 完整能力闭环 | 先复用和组合，有缺口才构建；形成可追溯产物、分层试验、范围化采用及结果复盘；[总设计](../DESIGN.md#capability-development) | 不默认每个问题都生成工具或把构建成功等同任务能力；目标是开放扩展，非无限资源保证 |
...
选择理由是使用现有 JavaScript SDK／包及库已有 RPC 支持；不要求把 npm 能力改写为 Go／Rust，也不强制所有能力转为 Node。当前 Node SDK 的 WebSocket 接入与 Go／Rust 的可选原生传输分别装配，不假定全部传输对等。库依据见[固定快照](go-mini-integration.md#javascript-rpc-facts)，工程方式见[Node 接入](engineering-reference.md#node-rpc-integration)；本次仅完成资料核对与文档接入，没有新增实验、npm 安装或产品实现。

<a id="active-learning-extension"></a>

## 主动学习与自我迭代的确认范围

2026-09-20 用户认可方案并要求写入，明确增加学习强度限制，最低可关闭、不主动学习。扩展存在、职责和强度控制为已定设计；具体学习策略与收益具有实验性，本次不授权实施或实验。
...
资料支持课程、反馈记忆及代码迭代的分工，不证明本项目长期成长收益、最佳强度或任意任务自我提升。具体预设名称、数值、初始强度、探索比例、课程算法及可选训练框架按用途配置，不列作当前架构未关闭项。不将认可解释为允许修改本轮评价规则、自动扩大权限或运行真实学习试验。资料与替代项见[学习依据](whole-system-evidence.md#active-learning-sources)。

<a id="initialization-extension"></a>

## 提示词初始化与个体自迁移的确认范围

2026-09-21 用户认可方案并要求写入和接入文档。以下为已定设计，仍未进入实现或实验；不将提示词或 API 修订标识用作 TinyAGI 产品阶段编号。
...
2026-09-21 用户认可最终方案并要求写入。预制库随核心客户端交付与更新，权威定义和实现对 Self 只读；Self 可选择使用、组合、独立包装或不使用。维护权、调用权限和实际作用分别定义，必要运行约束仍由宿主执行。范围覆盖初始化、正常认知、维护及管理，不限定为首次生成工具。

逻辑及选择理由见[总设计](../DESIGN.md#prebuilt-capabilities)，详细职责见[运行契约](runtime-protocol.md#prebuilt-library)，客户端与库装配见[工程参考](engineering-reference.md#client-library)。[管理入口](management-workspace.md#prebuilt-management)区分预制定义、使用配置及个体包装；[沙箱评价](sandbox-evaluation.md#prebuilt-evaluation)覆盖实际依赖和共同契约。新增能力供个体选用，已依赖实现的更新按兼容政策及实际迁移处理，未采用强制统一人格或自动改写个体程序的方式。

固定提交的[库依据](go-mini-integration.md#prebuilt-library-facts)支持源码装配、文档与语言工具接入；维护归属来自用户要求，初始化收益属于工程判断。具体函数清单、模块前缀和参数在实际接入时确定，不新增待执行实验；当前没有产品实现或新增效果实测。

<a id="task-control-extension"></a>

## 核心／任务解耦与外部控制的确认范围

2026-09-21 用户认可最终设计：主体、业务任务、认知执行和外部操作生命周期分开，任务超时／失败不结束 Self 或长期占用核心入口；长工作独立管理，结果沿事件接续。控制台、获授权用户及未来安全审计模块可通过宿主直接打断执行、阻断推进或中止任务，不等待认知批准。
...
原报告 014 的异步 FFI 与顺序跨实例接续不覆盖慢任务期间处理新输入、审计中止及故障隔离的完整场景。当前状态为设计已定、接入与实测未完成；这些适用边界不自动重开原 Q 队列。具体容量、期限及审计规则按用途或后续明确需求确定。

<a id="architecture-consistency"></a>

## 整体审计后的统一决策

2026-09-21 用户认可整体文档审计问题并要求一次性修订。以下为已定设计，统一消除职责与时序歧义；属于基于既有资料、库事实与用户目标的设计判断，不新增实验成绩，也不将此前未确认的完整副本重建细则改记为已认可。
...
| 沙箱范围与建议状态 | 通用局部试验按需装配；完整副本请求按既有建议继承完整授权范围。工作台示例注明依赖建议，不扩大用户已确认范围 | 不以最小任务切片冒称完整副本，不在整理时自动确认重建细则；[副本建议](sandbox-evaluation.md#replica) |

对应检查场景集中于[跨契约评价规格](sandbox-evaluation.md#contract-consistency-evaluation)。以上修订不重开 Q1–Q7，也不触发产品实现或真实验证。

<a id="next-steps"></a>

## 后续维护

设计整理不自动安排实验或进入实现。只有新需求、直接相反证据，或用户明确要求测试／实施时，才登记具体受影响的设计与工作范围；不因“尚无实际性能数据”反复重开全部 Q 项。

<a id="evidence-levels"></a>
<a id="experiment-rules"></a>

当前按[维护规范](documentation-guide.md#workflow)保留资料日期、版本、命题与推论，检查总图、职责、场景和链接的一致性。如果以后另行开展实验，预登记、冻结、失败保留和证据分类规则继续适用；不能改写旧输入和成绩。

<a id="historical-research"></a>

**历史研究编号与旧引用（不属于当前研究队列）**

下列锚点仅让旧报告继续可查。旧材料中的修订标签指实验输入，不是产品版本；当前阅读使用上方决策台账。

| 原入口 | 已有材料 |
| --- | --- |
| <a id="c1b"></a>活动评价环境 | [报告 011](whole-system-evidence.md#report-011) |
| <a id="c1c"></a>投影、修订与提醒 | [报告 012](whole-system-evidence.md#report-012) |
| <a id="c1d"></a>资料发现 | [报告 013](whole-system-evidence.md#report-013) |
| <a id="c1e"></a>单宿主组合 | [报告 014](whole-system-evidence.md#report-014) |
| <a id="e0"></a>E0 动作不确定性 | [原规格及适用范围](archive/mechanism-research.md#e0) |
| <a id="e1"></a>E1 连续性 | [原规格及适用范围](archive/mechanism-research.md#e1) |
| <a id="e2"></a>E2 并发提交 | [原规格及适用范围](archive/mechanism-research.md#e2) |
| <a id="e3"></a>E3 多心智 | [原规格及适用范围](archive/mechanism-research.md#e3) |
| <a id="e4"></a>E4 记忆检索 | [原规格及适用范围](archive/mechanism-research.md#e4) |
| <a id="e5"></a>E5 实时干扰 | [原规格及适用范围](archive/mechanism-research.md#e5) |
| <a id="e6"></a>E6 经验复用 | [原规格及适用范围](archive/mechanism-research.md#e6) |
| <a id="e7"></a>E7 演化 | [原规格及适用范围](archive/mechanism-research.md#e7) |
| <a id="e8"></a>E8 历史系列 | [原规格及适用范围](archive/mechanism-research.md#e8) |
| <a id="e8b"></a>E8b 进程与事务 | [原规格及适用范围](archive/mechanism-research.md#e8b) |
| <a id="e8c"></a>E8c 提供者核对 | [原规格及适用范围](archive/mechanism-research.md#e8c) |
| <a id="e8d"></a>E8d 接纳与结算 | [原规格及适用范围](archive/mechanism-research.md#e8d) |
| <a id="e8e"></a>E8e 取消与矛盾证据 | [原规格及适用范围](archive/mechanism-research.md#e8e) |
| <a id="s1"></a>S1 本地持久化 | [原规格及适用范围](archive/mechanism-research.md#s1) |
| <a id="s1a"></a>S1a 已测范围 | [原规格及适用范围](archive/mechanism-research.md#s1a) |
