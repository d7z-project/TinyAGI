...
| 日常管理 | [能力构建](#capability-management) · [学习强度](#learning-management) · [初始化与迁移](#initialization-management) · [预制能力](#prebuilt-management) · [任务控制](#task-control-management) · [隔离环境](#execution-environments) |
| 资源与机密 | [资源详情与操作](#resource-management) · [机密归属与使用](#secret-projection) · [可信提交](#secret-command) · [处理流程](#secret-pipeline) |
...
| 记忆与资源 | 记忆业务对象及各类资源的描述、表示、来源、处理范围和索引覆盖；按类型提供读取、使用、修订、导入导出或删除 |
...
资源详情依据[共同资源契约](projection-input-and-compute.md#resource-contract)展示当前可见的描述、归属、表示、来源、关联、处理范围、生命周期及支持操作。管理者和认知访问同一权威来源，但按各自身份形成投影；页面和目录不保存第二份业务状态。

普通属性按维护权限编辑；资源参数以有类型引用选择，Secret 使用固定可信控件。归属调整、凭据替换和业务状态变更通过明确的类型／领域操作处理，不能借属性编辑取消强制限制。没有通用编辑能力时，显示受限描述和专门操作，不伪造万能表单。

记录分别展示当时的投影、实际读过的材料、本次认知输入、执行组件使用和实际交付。加载过程序或使用过凭据不显示为认知已读；浏览详情也不把内容自动加入活动。来源、指令权限及事实评价分开，不提供一个同时改写它们的可信度开关。

类型结构、操作说明和返回类别与处理实现关联一致修订，页面按有效描述生成。描述及授权变更提交接纳后约束新操作；转换、构建或清理立即进入对应工作并报告实际进度，不将请求已接纳当作处理已完成。代码及普通配置的切换仍遵守[提交生效规则](#apply)。
...
请求详情关联实际实现、父活动、身份与执行域、所需及有效的文件／网络／资源限制、来源修订和最近检查时间。缺少条件时说明受影响操作及可选替代能力，其他合格任务仍可运行；不提供将缺失环境自动替换为普通执行的开关。底层提供者及物理配置见[工程参考](engineering-reference.md#container-execution)。

有权限者可维护提供者连接、来源与可用模板，调整允许目标、资源上限、启用状态及作用范围；参数由类型页面约束，不将任意底层管理权限交给自然语言辅助或候选程序。执行要求与资源维护权分别核对，修改描述不能扩大授权。提交沿[立即生效规则](#apply)处理；需要准备、启动或检查的工作立即登记并展示进度，只有实际可用才显示可用，不另设再次发布步骤。

运行列表显示本次接纳状态、实际用量、产物及收尾。关闭环境或收紧策略时显示受影响的新增请求和在途任务，按已有[任务控制](#task-control-management)处理其范围；停止请求、控制接纳、实际停止与远端效果分别展示。提供者失联显示未知，不能自动改成全部已停止。

执行配置、基础材料、日志和结果使用同源资源页面及当前投影；Secret 只显示引用和绑定状态，仍使用可信输入与专用认证通路。沙箱页面可以选择合格执行环境，但不把环境可用显示为 AGI 副本已完整重建或已通过能力评价。
...
## 5. Secret 的归属、可信输入与使用

Secret 的公共属性、引用、投影与原值限制统一见[资源类型规则](projection-input-and-compute.md#secret-resource)。本节维护用户归属、可信提交、专用操作及处理交互；原值由机密职责保管，模型只见当前任务必要的引用、用途和已配置／缺失／禁用／过期等状态。外部服务可在授权的认证通道接收凭据，模型提示与可见正文不承载凭据。
...
| 使用授权 | 明确哪个活动／实际能力实现在什么执行域、目标服务、操作和期限内使用；保管机密不自动授权一切任务使用 |
...
规则按字段类型、来源和目标契约组织，可在管理面板查看和调整相应映射，但不能通过通用表单、蓝图或停用某条遮蔽规则取消“原值不进入模型”的约束。Secret 原值及派生认证材料随转换保留机密性，编码不是脱敏；认证是[运行使用关系](projection-input-and-compute.md#resource-derivation)，不自动将全部业务结果标为 Secret。返回字段按目标契约分类，不能安全分离的内容留在受限通路，向普通业务只返回状态。
...
能力请求按当前调用者、归属、实际能力实现、执行域、操作、目标与绑定核对；受信执行职责解析凭据并在目标协议规定的位置使用。模型不能把合法引用改配到任意目标或回显工具，目标变化及重定向不自动继承认证。不存在向模型、通用资源读取或认知代码返回 SecretValue 的能力。
...
管理者查看资源描述取得情境投影，展开获准内容才是一次读取；操作按钮不保证存在内容回读入口。将材料加入某活动需要明确选择并核对该活动的处理范围，不自动继承管理会话权限。[资源视图](#resource-management)
...
资源字段沿[类型及操作描述](#resource-management)选择引用、允许的表示及范围，处理实现与描述一致采用。资源类型扩展只增加已声明的结构和操作，不通过自定义控件读取 Secret 原值，也不从新增字段推导敏感权限。
