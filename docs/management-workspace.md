...
目录：[目标](#purpose) · [逻辑职责](#architecture) · [管理范围](#coverage)／[能力构建管理](#capability-management)／[学习强度管理](#learning-management)／[初始化与迁移](#initialization-management) · [预制能力](#prebuilt-management) · [任务控制](#task-control-management) · [身份与范围](#identity) · [机密机制](#secret-projection) · [编辑与观察](#editing) · [副本与动态编辑](#revision-review)／[函数控制台](#function-console) · [完整场景](#scenarios) · [依据](#sources)
...
区分 `SecretValue`（原值）、`SecretRef`（标识引用）、`SecretProjection`（范围化元数据）、`SecretBinding`（使用者、执行域、目标和用途授权）与 `SecretIntake`（有身份和用途绑定的提交请求）。这些是逻辑概念，字段名称尚未冻结。引用不是通行证，知道标识不能读取或任意使用凭据。
...
| 保管与投影 | 保存原值和独立授权关系，将业务配置转换为 SecretRef；收集成功与服务认证可用分别表达 | 请求完成、已配置及范围化投影，不声称已验证上游有效性 |
...
对误粘贴到普通聊天、附件或外部结果中的机密，能确定的部分在入模前替换为无原值提示，无法安全切分的可疑内容隔离并请求专用提交；不默默将不确定文本保存成用户的凭据。此补充检测无法保证识别所有未知秘密，不能作为主要隔离机制。主体后续思考、摘要或日志过滤均不能撤回已经发送给模型的内容。
...
本节维护操作方式；编写契约与副本重建细则的不同确认范围统一见[台账](research-plan.md#management-extension)。
...
以下依照仍属建议的[副本重建机制](sandbox-evaluation.md#replica)作条件化走查，不作为机制已获确认或已实现的记录。管理者从正式主体当前业务状态创建暂停的副本 A，切换后仍使用同一套主体、活动、资源和程序页面。A 保留基准时点的记忆与任务，输出装配为试验收件箱。
...
甲可在“我的凭据”替换或撤销。用户乙提出同类任务不会匹配到甲的同名凭据；管理员可以按权限更正归属，旧授权随归属变更重新核对。工作台显示每一步的无原值状态，主体不接触页面访问凭证、输入值或认证回显。这是逻辑走查，没有创建真实入口或调用外部服务。
