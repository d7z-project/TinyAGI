...
[文档索引](../README.md#experiments) · [当前设计](../../DESIGN.md#evidence) · [研究问题](../research-plan.md#e8b)
...
前一轮 E8 的权威与原子效果来自模型假设。本轮将指派复核、去重和实际计数器落入真实数据库，直接终止工作进程并重新连接检查，检验哪些“持久化”边界足以覆盖这些切点。Kubernetes 已按用户要求固定为平台，本实验验证其上层应用协议的一部分。
...
预登记：[README](../../experiments/process-recovery/README.md)。脚本：[experiment.py](../../experiments/process-recovery/experiment.py)。原始结果：[process-recovery.json](../../experiments/results/process-recovery.json)，包含各子进程 PID、BootID、事件、退出码、数据库观察和判定。
...
复现命令见 README；输出已存在时脚本拒绝覆盖，复跑须选择新路径。此次没有执行失败或修改实验后重跑。
...
原子组：计数器与回执在同一数据库事务中提交。分离组：先提交 world 数据库的计数器，再提交 provider 数据库的回执，模拟提供者记录与外部效果不能共同提交的边界。两组的 ResourceEpoch=2 均已预先持久化；本轮不测试安装代号的接管过程。
...
下一步仍需真实复制后端与隔离 Kubernetes 集群中的故障验证，以及运行中动作排空、旧备份恢复和非原子提供者的核对协议。Kubernetes 控制器、CNI／CSI、节点分区、卷迁移及真实业务状态迁移均未在本轮测试。

结论已回写[总设计](../../DESIGN.md#distributed-runtime)、[Kubernetes 设计](../kubernetes-runtime.md)、[分布式协议](../distributed-runtime.md)及[运行协议](../runtime-protocol.md)。
