...
本篇说明资源怎样被发现和读取、外部事件怎样接续活动，以及认知如何使用计算与缓存。资源默认以投影呈现，内容由主体按需读取；推理和 KV 缓存由计算组件管理。闹钟仅用于举例说明外部通知源，不是内置模块。

已有[本地流程实验](experiments/012-projection-alarm-activity.md)和[资料发现对照](experiments/013-resource-discovery.md)，支持范围见文末；KV 性能和真实通知效果尚无本项目实测结论。

[项目入口](../README.md) · [文档索引](README.md) · [总架构](../DESIGN.md#logical-architecture) · [整体研究](research-plan.md#whole-system)

目录：[整体分工](#architecture) · [投影与读取](#projection) · [外部事件源与通知示例](#event-sources) · [思考与简短交流](#effort) · [缓存与 KV](#cache) · [取舍](#alternatives) · [资料范围](#sources) · [适用边界](#validation)
