...
执行环境、程序包、构建输入和运行产物沿同一资源契约描述。环境要求关联具体实现、用途与操作；可见的能力声明、最近检查和本次有效约束分别投影，目录不另存一套准入真值。获准将资源交给隔离组件处理，不扩大其外部发送或 Secret 使用资格；镜像等运行资产的内容标识也不等于来源可信。环境缺失及隔离不足按[执行准入](runtime-protocol.md#isolated-execution)处理。
