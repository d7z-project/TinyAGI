...
[总设计](../DESIGN.md#decisions) · [文档索引](README.md) · [设计收尾与边界](research-plan.md)

这里统一维护已有实验、原始资料和一手来源。整理后的报告可公开，原始测试材料保留在本地忽略目录；网站中的相关引用标明不发布，范围见[发布说明](documentation-publishing.md#local-materials)。报告编号只用于定位证据；旧文件中的版本标签标识当时冻结的材料，不代表 TinyAGI 已有产品版本。依赖库、论文修订、数据／协议版本保留各自的真实含义。

目录：[整体任务实验](#whole-task-experiments) · [机制实验](#mechanism-experiments) · [设计走查](#development-tasks) · [按问题查依据](#topics) · [设计机制依据](#design-closure) · [主动学习依据](#active-learning-sources) · [任务解耦与控制依据](#task-control-sources) · [一致性审计依据](#consistency-basis) · [既有论文与标准](#sources)

<a id="results"></a>
历史阅读提示：报告及原始材料保持形成时的名称、条件和失败。旧稿中的 `DESIGN v…` 是当时写稿标识，数据路径中的修订号用于定位冻结输入，均不代表现行产品版本；论文、标准及依赖的版本则用于精确引用。报告中的“下一步”只描述当时计划，当前状态以[设计台账](research-plan.md#research-status)为准。

<a id="experiments"></a>

## 实验证据

用户约束确定目标；实验只支持其实际运行条件内的结论。现有材料包括状态模型、预编排流程、真实本地组件、固定语料检索及受限真实模型开发对照；**尚无独立保留任务上的稳定收益、正式能力基线或真人体验证据**。具体统计在原报告，当前设计的采用理由见总设计和专题。

<a id="whole-task-experiments"></a>

### 整体任务相关实验

| 报告 | 实际验证范围 | 不能由此证明 | 原始资料 |
| --- | --- | --- | --- |
| <a id="report-011"></a>011 · [完整活动评价](experiments/011-whole-activity-evaluation.md) | 固定阶段的活动输入、预编排产物和已知失败可被所列客观条件区分；语义盲区保持待审 | 无参试模型、真实联系人或语义质量成绩，不能证明工作区／情感／协作收益 | [登记、输入及脚本](../experiments/whole-activity-evaluation/README.md)；[逐轨迹输出与 SHA-256](../experiments/whole-activity-evaluation/results/README.md) |
| <a id="report-012"></a>012 · [投影、修订与提醒](experiments/012-projection-alarm-activity.md) | 本地文件、读取缓存和真实计时输入可与资料修订、产物交付及分别结束相接；旧证据与漏交付控制失败被保留 | 预编排策略和合成参数；不是模型判断、KV 性能、目录发现率或真实通知效果 | [预登记与脚本](../experiments/whole-activity-evaluation-v2/README.md)；[逐轨迹输出与 SHA-256](../experiments/whole-activity-evaluation-v2/results/README.md) |
| <a id="report-013"></a>013 · [资源发现与用途](experiments/013-resource-discovery.md) | 当前文档快照中，元数据与主动内容查询提供不同候选；用途范围可缩小搜索，命中仍须阅读核对 | 公开开发查询、词面算法及人工用途标签；不是模型理解、检索泛化或清理后的性能提升 | [预登记、快照与脚本](../experiments/resource-discovery/README.md)；[逐查询输出与 SHA-256](../experiments/resource-discovery/results/README.md) |
| <a id="report-014"></a>014 · [单宿主组合](experiments/014-host-composition.md) | 实际 Go／go-mini／SQLite／文件／HTTP 可在单程序接续简单完整任务；无依据、未交付和错误请求控制有区分 | 确定提供者、虚拟时间及有限域装配；没有模型质量、完整沙箱、厂商接口或产品成绩 | [预登记、场景及独立模块](../experiments/host-composition/README.md)；[完整运行与 SHA-256](../experiments/host-composition/results/README.md) |
| <a id="report-015"></a>015 · [任务验收与模型对照](experiments/015-task-outcome-contract.md) | 合理路径／过程失败控制；两模型在合成资料任务中的读取、更正、交付与打断；内容审阅发现历史解释问题 | 单任务结构、每条件一次；非完整沙箱／攻击测试，无广泛组织收益、真人体验或正式采用结论；服务修复前失败另列 | [冻结输入](../experiments/task-outcome-contract/README.md)、[修复后重测](../experiments/task-outcome-contract/RETEST.md)、[结果与评阅](../experiments/task-outcome-contract/results/README.md) |
| <a id="report-016"></a>016 · [功能事件模型验证](experiments/016-functional-events.md) | 两模型的三类修订、多活动撤销、账号证据／群聊受众及外部通知成功／失败；揭示提前让出、预算截断、过早完成与内容补写 | 固定提示、合成环境、少量重复、单助手评阅；无真实通知／跨平台认证、正式基线或架构优越性结论 | [冻结输入](../experiments/functional-events/README.md)、[模型轨迹](../experiments/functional-events/results/models-001/summary.json)、[补充评阅](../experiments/functional-events/results/review-001/assessment.json) |
| <a id="report-017"></a>017 · [接续、投入与责任退出](experiments/017-continuation-study.md) | 接续语义提示包、投入档位／生成上限分别对照；新增未裁定冲突与给定意愿退出；另登记责任接纳补充 | 小样本与单助手评阅；提示不等于宿主保证，网关参数兼容不等于透传，退出处理不证明真实偏好形成 | [主登记](../experiments/continuation-study/README.md)、[补充登记](../experiments/continuation-study/GUARD.md)、[评分范围审计](../experiments/continuation-study/EVALUATOR-AUDIT.md) |
...
| 报告 | 实际验证范围 | 不能由此证明 | 原始资料 |
| --- | --- | --- | --- |
| <a id="report-001"></a>001 · [动作恢复](experiments/001-effect-recovery.md) | 超时／崩溃后可能无法区分外部效果 0／1，盲重试会重复 | 有限状态模型不是实际服务保证 | [脚本](../experiments/effect_recovery_model.py)；条件见报告；[JSON](../experiments/results/effect-recovery.json) |
| <a id="report-002"></a>002 · [VM 连续性](experiments/002-vm-continuity.md) | VM 状态需恢复、补丁需业务边界、scope 和旧引用需管理 | 合成状态不能证明模型语义连续性 | [实验说明](../experiments/vm-continuity/README.md)；[JSON](../experiments/results/vm-continuity.json) |
| <a id="report-003"></a>003 · [结果复核](experiments/003-result-revalidation.md) | 目标版本和串行写入不足以代表全部依赖 | 手写条件不能证明自动捕获完整 | [预登记](../experiments/result-revalidation/README.md)；[JSON](../experiments/results/result-revalidation.json) |
| <a id="report-004"></a>004 · [控制通路](experiments/004-realtime-control.md) | 接纳、调度、执行和输出分别约束；取消不等于资源已释放 | 模拟 tick 不是设备延迟 | [预登记](../experiments/realtime-control/README.md)；[JSON](../experiments/results/realtime-control.json) |
| <a id="report-005"></a>005 · [分布式接管](experiments/005-distributed-handoff.md) | 保留原报告用于历史追溯 | 接管、分区和多节点方案退出当前架构 | [预登记](../experiments/distributed-handoff/README.md)；[JSON](../experiments/results/distributed-handoff.json) |
| <a id="report-006"></a>006 · [进程恢复](experiments/006-process-recovery.md) | SQLite 事务边界与效果／回执分离的崩溃窗口 | 不采用其节点指派方案，不证明完整本地产品恢复 | [预登记与复现](../experiments/process-recovery/README.md)；[JSON](../experiments/results/process-recovery.json) |
| <a id="report-007"></a>007 · [提供者核对](experiments/007-provider-reconciliation.md) | 缺回执、键过期、取消与逐资源回收需分别解释 | 受控进程契约不能套用任意第三方服务 | [预登记](../experiments/provider-reconciliation/README.md)、[脚本](../experiments/provider-reconciliation/experiment.py)；[JSON](../experiments/results/provider-reconciliation.json)、[环境](../experiments/results/provider-reconciliation-environment.json) |
| <a id="report-008"></a>008 · [接纳与入账](experiments/008-dispatch-settlement.md) | 提交后确认、持久待办和唯一释放账 | 多个实验库是夹具，不是产品必须拆成多个库 | [预登记](../experiments/dispatch-settlement/README.md)、[脚本](../experiments/dispatch-settlement/experiment.py)、[修订及失败记录](../experiments/dispatch-settlement/REVISIONS.md)；[已冻结结果](../experiments/results/dispatch-settlement-v3.json) |
| <a id="report-009"></a>009 · [取消、恢复与冲突](experiments/009-cancel-restore-evidence.md) | 取消竞争、旧快照重复风险和冲突证据需处理 | 依赖给定可信来源与恢复条件，不证明完整灾备 | [预登记](../experiments/cancel-restore-evidence/README.md)、[脚本](../experiments/cancel-restore-evidence/experiment.py)；[JSON](../experiments/results/cancel-restore-evidence.json) |
| <a id="report-010"></a>010 · [本地存储与维护](experiments/010-local-storage.md) | 文件先发布后提交、稳定锁对象、维护排空及完整备份；长读影响写入与 checkpoint | 小型 SQLite／Linux 夹具，未测 Go 驱动、完整认知闭环、吞吐或掉电 | [预登记](../experiments/local-storage/README.md)、[脚本](../experiments/local-storage/experiment.py)；[JSON](../experiments/results/local-storage.json) |

<a id="development-tasks"></a>

## 设计走查材料

[完整活动设计走查登记](../experiments/whole-activity-design/README.md)及其[指南](../experiments/whole-activity-design/guide.md)、[研究](../experiments/whole-activity-design/research.md)、[观察](../experiments/whole-activity-design/watch.md)、[走查](../experiments/whole-activity-design/review.md)为设计材料，不是模型运行结果；[算术](../experiments/whole-activity-design/arithmetic.json)和[哈希](../experiments/whole-activity-design/manifest.json)按原条件保存。

<a id="context"></a>
<a id="collaboration"></a>
<a id="learning"></a>
<a id="quick-decisions"></a>
...
2026-09-21 的[统一决策](research-plan.md#architecture-consistency)来自对当前总设计、六份专题、工程映射、库记录和既有证据边界的交叉审查。步骤内独立操作接纳、带类型控制、公共决策整合、配置与当前约束、个体状态及实现信任是本项目消除冲突的设计判断；本次只修改和检查文档，没有新增模型、组件或端到端验证。

既有[结果复核](experiments/003-result-revalidation.md)、[接纳与入账](experiments/008-dispatch-settlement.md)和[取消与冲突](experiments/009-cancel-restore-evidence.md)支持保留归属、依赖和实际作用的动机，不证明新增组合协议已经运行。库执行、MRPC 和生命周期事实沿[固定提交记录](go-mini-integration.md#library-facts)解释；早期沙箱接口的工作区摘要统一见[历史读取](go-mini-integration.md#sandbox-library-facts)，当前文件链接不充当固定原件。人格整合与学习自主性来自用户目标及设计取舍，不伪装成已测认知收益。
...
| 上下文与任务接续 | [活动工作区](whole-system-design.md#workspace) | [上下文设计走查](archive/design-studies.md#whole-system-evidence-context)，P1 |
| 心智协作 | [协作机制](whole-system-design.md#organization) | [协作推导](archive/design-studies.md#whole-system-evidence-collaboration)，P2；[多人格分因素对照](archive/design-studies.md#multi-persona-discussion-review-evaluation) |
| 交付与经验 | [学习](whole-system-design.md#memory) | [反馈及反思](archive/design-studies.md#whole-system-evidence-learning)，P3–P5 |
| 何时继续、如何读取和评价 | [任务循环](whole-system-design.md#loop) | [原验收分析](archive/design-studies.md#whole-system-evidence-quick-decisions)，P6–P8 |
| 关注、计划与程序复用 | [目标与计划](../DESIGN.md#attention-planning) | [原任务核对](archive/design-studies.md#whole-system-evidence-attention-planning)，P9–P11 |
| 来源修订、路由与版本采用 | [来源与能力](../DESIGN.md#knowledge) | [原分析](archive/design-studies.md#whole-system-evidence-evidence-routing-evolution)，P12–P14 |
...
| 统一事件与活动接续 | [事件主线](../DESIGN.md#event-driven)、[共同协议](runtime-protocol.md#events) | [用户认可、CloudEvents 参考与证据边界](runtime-protocol.md#event-sources-evidence)；报告 012／014 为局部流程，016／017 补充受控来源与接续对照；完整体系未测 |
...
**查阅／复核日期：2026-09-20。** 本节是原 Q1–Q7 的资料论证记录，全部按[设计决策台账](research-plan.md#remaining-questions)收尾。这份资料记录来自检索和阅读，不包含新增模型调用、实验或作者结果复现。固定论文修订与标准日期；在线厂商文档仅代表查阅日所见契约，没有锁定本项目安装版本。下面分开来源命题与本项目推论，未列出的性能、可靠性和真人效果不在支持范围。
...
| 来源及定位 | 资料支持什么 | 采用推论与适用边界 |
| --- | --- | --- |
| [Scaling Agent Systems，修订 3，2026-04-08](https://arxiv.org/abs/2512.08296v3)，摘要及限制 | 控制工具、提示和计算后，协调效果随任务结构变化，工具密集任务存在额外开销 | 默认单负责心智，按独立贡献需求邀请；不固定团队规模或宣称所有任务提分 |
...
长期人格存在由已认可的主体目标决定；其经历、意愿与权限不由角色标签替代。临时 Mind、长期 Persona、对外 Role 和计算模型仍是不同概念。角色提示与长期行为的原始依据见[既有协作资料](archive/design-studies.md#multi-persona-discussion-review-sources)，没有把人格性能优越性列为默认假设。
...
两阶段复盘、逐能力门槛、固定参考和有限采用是本项目结合反馈学习及 Q1 评价原则的设计。该机制选择已收尾，没有新迁移成绩。获准的主动学习与候选采用见下方补充，不从资料本身推导执行授权或模型权重修改。
...
采用“稳定宿主支持＋可替换学习策略”，学习方向可以来自兴趣和能力观察；候选保留与生产采用分别判断，评价检查后续使用和全部成本。最低强度可关闭、宿主执行预算及停止、用户明确学习任务与自发学习区分，均属于用户要求与工程／逻辑设计，不能声称由论文证明最佳强度。

不采用只有用户逐项点名才学习、固定未知度越高越优先、所有探索立即提分、只靠自评分采用或必须修改模型权重；分别因为限制主动性、混淆不可学习问题、抹去中间研究价值、缺少独立依据或增加无必要前提。已认可范围见[台账](research-plan.md#active-learning-extension)，机制见[主体专题](whole-system-design.md#active-learning)；没有运行新实验或模型调用。

<a id="closure-interaction"></a>

### 身份、隐私、情感和真实交互（Q2／Q3／Q7）
...
强自主回应、长期人格和虚拟伴侣支持范围来自用户选择；资料为情境组织、证据分工及可撤销交互提供支撑。具体法律要求绑定实际法域与用途，沿用[规范设计](interaction-design.md#norms)；不以通用治理资料代替法律文本。
...
<a id="management-sources"></a>

管理工作台的新增副本／动态编辑设计依据见[管理专题资料表](management-workspace.md#sources)，2026-09-20 查阅 JSON Forms、Node-RED、LangGraph、Ollama 官方文档及 OWASP Secrets Management；补充模型维护、secret 投影，以及用户归属、专用指令和过滤转换流程；同日补查 OWASP Forgot Password 与 W3C Secure Contexts，分别借鉴一次性受限会话和可信来源的适用边界；go-mini 固定提交的只读核对见[库接入参考](go-mini-integration.md#management-library-review)。来源支持相关分工；当前认可范围见[台账](research-plan.md#management-extension)，未增加实验成绩。

<a id="hot-reload-sources"></a>

2026-09-20 用户认可按编写契约热加载、类型／结构化注释驱动适配与受管理函数调用。依据包括同日核对的 [Go 特殊注释与生成方式](https://go.dev/blog/generate)及 [go-mini 固定提交入口／补丁契约](go-mini-integration.md#managed-entry-facts)；支持声明、适配和边界分工，不证明生成工具已实现、任意函数均可适配或任意状态均可原地热更。逻辑选择见[运行契约](runtime-protocol.md#managed-functions)，具体映射及未采用方案见[工程参考](engineering-reference.md#managed-function-tooling)，未新增实验成绩。

<a id="capability-extension-sources"></a>

2026-09-20 的能力构建与接入设计采用 MRPC 多语言绑定、局部实例试验和宿主管理原生程序。go-mini 固定提交、已确认 API 与未确认的子进程服务边界见[库接入记录](go-mini-integration.md#rpc-extension-facts)；Go os/exec 与 Cargo 构建脚本的资料命题、工程推论及未采用项见[工程参考](engineering-reference.md#capability-toolchain)。本轮只读资料并修订设计，未运行构建、RPC、模型或新实验；不能把资料支持写成系统效果实测。

2026-09-21 按用户补充扩展 Node.js（npm）能力端。新增来源为 go-mini 固定提交的 [TypeScript 生成、Node RPC SDK 与双向互通测试源码](go-mini-integration.md#javascript-rpc-facts)，只支持库接口与接入方式；源码中存在测试不等于本次已运行。程序包／依赖固定与受管理 Node 服务属于[工程设计](engineering-reference.md#node-rpc-integration)，无新增安装、构建、模型或互通实验成绩。

<a id="initialization-migration-sources"></a>

### 提示词初始化与自迁移依据

2026-09-21 用户确认以当前完整初始化提示词及相邻差异引导人物形成和更新，允许模型／个体差异，并要求 API 弃用兼容。这是用户目标；不声称有论文证明所有模型会产生等价人物或可靠自迁移。
...
采用理由、未采用方案和边界见[工程参考](engineering-reference.md#prompt-bootstrap)，状态见[确认范围](research-plan.md#initialization-extension)。本次未运行初始化、编译、迁移或真实模型实验，旧实验没有因此新增覆盖。

<a id="prebuilt-library-sources"></a>

### 客户端预制能力库依据

2026-09-21 用户确认随核心客户端交付和更新预制能力，Self 按需选用、组合或不用，权威实现由客户端维护。该维护归属属于用户要求。相邻 go-mini 固定提交的[标准源码库注册、显式 provider 装配及语言查询接口](go-mini-integration.md#prebuilt-library-facts)支持工程接入；复用以减少机械性编写及接口猜测是工程判断，不是已测收益。逻辑见[预制能力](../DESIGN.md#prebuilt-capabilities)，承载及替代项见[工程参考](engineering-reference.md#client-library)，状态见[台账](research-plan.md#prebuilt-extension)。本轮只读资料和更新文档，未运行新实验。

<a id="task-control-sources"></a>

### 核心与任务解耦、外部中止的依据

2026-09-21 用户确认主体／任务生命周期解耦与控制台、授权审计等外部控制。只读核对 mini-go 固定提交的前台入口、异步等待、InterruptHandle、scope、实例故障及 RPC 取消，源码和测试用例身份见[库记录](go-mini-integration.md#task-control-facts)。同日查阅 [Go context 官方文档](https://pkg.go.dev/context#pkg-overview)（页面所示 go1.27.1），支持取消沿派生上下文传播的命题；宿主据此分别管理短请求、已接纳任务及关闭生命周期。

资料支持有界执行、受控取消与分责接入，任务授权、阻断状态及解除流程是 TinyAGI 设计。原[报告 014](experiments/014-host-composition.md)支持实际异步 FFI 和顺序跨实例接续，不支持本轮新增完整并发控制结论；库测试源码阅读也不写成新实测通过。未运行测试、模型或新实验，响应性与取消延迟未测。采用决定见[台账](research-plan.md#task-control-extension)，工程推论见[接入映射](engineering-reference.md#task-execution)。

<a id="next"></a>

当前收尾决定及适用边界统一见[设计台账](research-plan.md#remaining-questions)；本索引不安排新的实验任务。
